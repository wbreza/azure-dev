...
type ExternalServiceTarget struct {
	...
	streamManager *streaming.StreamRequestManager[*azdext.ServiceTargetMessage]
}
...
func (est *ExternalServiceTarget) Publish(
	...
	resp, err := est.streamManager.SendWithProgress(ctx, req, newProgressAdapter(progress))
	if err != nil {
		return nil, err
	}
...
func NewExternalServiceTarget(
	...
	target := &ExternalServiceTarget{
		...
		prompters:     prompters,
		env:           env,
		streamManager: streaming.NewStreamRequestManager[*azdext.ServiceTargetMessage](stream),
	}
...
func (est *ExternalServiceTarget) Initialize(ctx context.Context, serviceConfig *ServiceConfig) error {
	...
	_, err = est.streamManager.Send(ctx, req)
	return err
}
...
func (est *ExternalServiceTarget) Deploy(
	...
	resp, err := est.streamManager.SendWithProgress(ctx, deployReq, newProgressAdapter(progress))
...
func (est *ExternalServiceTarget) Endpoints(
	...
	resp, err := est.streamManager.Send(ctx, req)
	if err != nil {
		return nil, err
	}
